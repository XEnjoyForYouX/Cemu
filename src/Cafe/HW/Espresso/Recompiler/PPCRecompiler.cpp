...
#elif defined(__aarch64__)
	auto& gprPhysPool = raParam.GetPhysRegPool(IMLRegFormat::I64);
	for (int i = IMLArchAArch64::PHYSREG_GPR_BASE; i < IMLArchAArch64::PHYSREG_GPR_COUNT; i++)
		gprPhysPool.SetAvailable(i);

	auto& fprPhysPool = raParam.GetPhysRegPool(IMLRegFormat::F64);
	for (int i = IMLArchAArch64::PHYSREG_FPR_BASE; i < IMLArchAArch64::PHYSREG_FPR_COUNT; i++)
